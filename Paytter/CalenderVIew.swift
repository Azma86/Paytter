...
struct CalendarView: View {
            ...
            Button(action: {
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10)
                        .stroke(Color(hex: themeMain).opacity(0.3), lineWidth: 1)
                )
    ...
    func combinedDate() -> Date {
        let now = Date()
        var c = calendar.dateComponents([.year, .month, .day], from: selectedDate)
        let tc = calendar.dateComponents([.hour, .minute], from: now)
        c.hour = tc.hour
        c.minute = tc.minute
        return calendar.date(from: c) ?? selectedDate
    }
    ...
    func checkIsHoliday(_ date: Date) -> Bool {
        return getHolidayName(date) != nil
    }
