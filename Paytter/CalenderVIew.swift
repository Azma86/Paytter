    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                Divider()
                List {
                    if filteredTransactions.isEmpty { HStack { Spacer(); Text("投稿はありません").font(.caption).foregroundColor(.secondary).padding(.top, 40); Spacer() }.listRowSeparator(.hidden).listRowBackground(Color.clear) }
                    else {
