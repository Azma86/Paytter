...
struct CalendarView: View {
    ...
    var filteredTransactions: [Transaction] { transactions.filter { calendar.isDate($0.date, inSameDayAs: selectedDate) }.sorted(by: { $0.date > $1.date }) }

    var body: some View {
        VStack(spacing: 0) {
            HStack { Button(action: { moveMonth(by: -1) }) { Image(systemName: "chevron.left") }; Spacer(); Button(action: { tempPickerDate = currentMonth; isShowingMonthPicker = true }) { HStack(spacing: 4) { Text(monthYearString(from: currentMonth)).font(.headline).foregroundColor(.primary); Image(systemName: "chevron.down").font(.caption).foregroundColor(.secondary) } }; Spacer(); Button(action: { moveMonth(by: 1) }) { Image(systemName: "chevron.right") } }.padding(.horizontal).padding(.vertical, 8)
            HStack { ForEach(daysOfWeek, id: \.self) { day in Text(day).font(.system(size: 11, weight: .bold)).frame(maxWidth: .infinity).foregroundColor(day == "日" ? Color(hex: themeHoliday) : (day == "土" ? .blue : .primary)) } }.padding(.bottom, 5)
            GeometryReader { geometry in let width = geometry.size.width; HStack(spacing: 0) { monthGrid(for: calendar.date(byAdding: .month, value: -1, to: currentMonth)!, width: width); monthGrid(for: currentMonth, width: width); monthGrid(for: calendar.date(byAdding: .month, value: 1, to: currentMonth)!, width: width) }.offset(x: -width + dragOffset).contentShape(Rectangle()).gesture(DragGesture().onChanged { dragOffset = $0.translation.width }.onEnded { value in let threshold = width * 0.3; if value.translation.width < -threshold { withAnimation(.easeInOut(duration: 0.45)) { dragOffset = -width }; DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { currentMonth = calendar.date(byAdding: .month, value: 1, to: currentMonth)!; dragOffset = 0 } } else if value.translation.width > threshold { withAnimation(.easeInOut(duration: 0.45)) { dragOffset = width }; DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { currentMonth = calendar.date(byAdding: .month, value: -1, to: currentMonth)!; dragOffset = 0 } } else { withAnimation(.easeInOut(duration: 0.3)) { dragOffset = 0 } } }) }.frame(height: 280)
            Divider()
            List {
                if filteredTransactions.isEmpty { HStack { Spacer(); Text("投稿はありません").font(.caption).foregroundColor(.secondary).padding(.top, 40); Spacer() }.listRowSeparator(.hidden) }
                else { ForEach(filteredTransactions) { item in ZStack { NavigationLink(destination: TransactionDetailView(item: item, transactions: $transactions, accounts: $accounts)) { EmptyView() }.opacity(0); TwitterRow(item: item) }.listRowInsets(EdgeInsets()).swipeActions(edge: .trailing, allowsFullSwipe: false) { Button { transactionToDelete = item; isShowingDeleteAlert = true } label: { Text("削除") }.tint(.red) } } }
                Button(action: { self.inputText = ""; self.isShowingInputSheet = true }) { HStack { Image(systemName: "plus"); Text("投稿を作成") }.font(.subheadline).fontWeight(.bold).frame(maxWidth: .infinity).padding(.vertical, 12).background(Color.white).foregroundColor(Color(hex: themeMain)).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color(hex: themeMain).opacity(0.3), lineWidth: 1)).padding(.horizontal, 40).padding(.vertical, 20) }.listRowSeparator(.hidden)
            }.listStyle(.plain)
        }
        .navigationTitle("カレンダー").navigationBarTitleDisplayMode(.inline)
        .alert("投稿を削除しますか？", isPresented: $isShowingDeleteAlert) { Button("キャンセル", role: .cancel) { }; Button("削除", role: .destructive) { if let t = transactionToDelete, let idx = transactions.firstIndex(where: { $0.id == t.id }) { withAnimation(.easeOut(duration: 0.2)) { transactions.remove(at: idx) } } } } message: { if let t = transactionToDelete { Text(t.cleanNote) } }
