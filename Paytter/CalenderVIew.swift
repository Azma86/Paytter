...
struct CalendarView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                GeometryReader { geometry in
                    ...
                    HStack(spacing: 0) {
                        ...
                        monthGrid(for: calendar.date(byAdding: .month, value: 1, to: currentMonth)!, width: width)
                    }.offset(x: -width + dragOffset).contentShape(Rectangle()).gesture(DragGesture().onChanged { dragOffset = $0.translation.width }.onEnded { value in handleDragEnded(value: value, width: width) })
        ...
        .sheet(isPresented: $isShowingInputSheet) { PostView(inputText: $inputText, isPresented: $isShowingInputSheet, initialDate: combinedDate(), isExcludedInitial: false, onPost: handlePostTransaction, transactions: transactions, accounts: accounts) }
    ...
    private var headerView: some View {
        VStack(spacing: 0) {
            ...
            HStack { ForEach(daysOfWeek, id: \.self) { day in Text(day).font(.system(size: 11, weight: .bold)).frame(maxWidth: .infinity).foregroundColor(day == "日" ? Color(hex: themeHoliday) : (day == "土" ? Color(hex: themeSaturday) : Color(hex: themeBodyText).opacity(0.8))) } }.padding(.bottom, 8)
        }.background(Color(hex: themeBarBG).opacity(0.4))
    }
    
    private var dateHeaderView: some View {
        HStack(spacing: 4) {
            let holidayName = getHolidayName(selectedDate); let isSunday = calendar.component(.weekday, from: selectedDate) == 1; let isSaturday = calendar.component(.weekday, from: selectedDate) == 7; let isHoliday = holidayName != nil
            let dayColor: Color = (isHoliday || isSunday) ? Color(hex: themeHoliday) : (isSaturday ? Color(hex: themeSaturday) : Color(hex: themeBodyText))
            Text(formatDate(selectedDate, format: "yyyy年M月d日")).foregroundColor(Color(hex: themeBodyText))
            (Text("(").foregroundColor(Color(hex: themeBodyText)) + Text(formatDate(selectedDate, format: "EEE")).foregroundColor(dayColor) + Text(")").foregroundColor(Color(hex: themeBodyText)))
            if let name = holidayName { Text(name).foregroundColor(Color(hex: themeBodyText)).padding(.leading, 4) }
            Spacer()
        }.font(.footnote).fontWeight(.bold).padding(.horizontal).padding(.vertical, 6).background(Color(hex: themeBarBG).opacity(0.2))
    }
    
    private var transactionListView: some View {
        List {
            if filteredTransactions.isEmpty { HStack { Spacer(); Text("投稿はありません").font(.caption).foregroundColor(Color(hex: themeSubText)).padding(.top, 40); Spacer() }.listRowSeparator(.hidden).listRowBackground(Color.clear) } else {
                ForEach(filteredTransactions) { item in ZStack { NavigationLink(destination: TransactionDetailView(item: item, transactions: $transactions, accounts: $accounts)) { EmptyView() }.opacity(0); TwitterRow(item: item) }.listRowInsets(EdgeInsets()).listRowBackground(Color(hex: themeBG)).swipeActions(edge: .trailing, allowsFullSwipe: false) { Button { transactionToDelete = item; isShowingDeleteAlert = true } label: { Text("削除") }.tint(.red) } }
            }
            Button(action: { self.inputText = ""; self.isShowingInputSheet = true }) { HStack { Image(systemName: "plus"); Text("投稿を作成") }.font(.subheadline).fontWeight(.bold).frame(maxWidth: .infinity).padding(.vertical, 12).background(Color(hex: themeBG)).foregroundColor(Color(hex: themeMain)).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color(hex: themeMain).opacity(0.3), lineWidth: 1)).padding(.horizontal, 40).padding(.vertical, 20) }.listRowSeparator(.hidden).listRowBackground(Color.clear)
    ...
    @ViewBuilder func monthGrid(for month: Date, width: CGFloat) -> some View {
        ...
        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 7), spacing: 0) { ForEach(0..<allDays.count, id: \.self) { index in dayCell(date: allDays[index], month: month) } }.frame(width: width).background(Color(hex: themeBG))
    }
    ...
    func deleteTransaction() { if let t = transactionToDelete, let idx = transactions.firstIndex(where: { $0.id == t.id }) { transactions.remove(at: idx); NotificationCenter.default.post(name: NSNotification.Name("SwitchToHomeTab"), object: nil) } }
    ...
    func handlePostTransaction(isInc: Bool, date: Date, isExc: Bool, profileId: UUID?) { transactions.append(Transaction(amount: parseAmount(from: inputText), date: date, note: inputText, source: parseSourceName(from: inputText), isIncome: isInc, isExcludedFromBalance: isExc, profileId: profileId)); NotificationCenter.default.post(name: NSNotification.Name("SwitchToHomeTab"), object: nil) }
