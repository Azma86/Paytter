...
struct CalendarView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                // コンパイラ負荷軽減のため、日付表示部分をサブビューに分離
                dateHeaderView
                
                // コンパイラ負荷軽減のため、リスト部分をサブビューに分離
                transactionListView
            }
        }
        ...
        .sheet(isPresented: $isShowingInputSheet) {
            // 【変更】計算除外機能（isExcludedInitial, isExc）を引き継いで追加
            PostView(inputText: $inputText, isPresented: $isShowingInputSheet, initialDate: combinedDate(), isExcludedInitial: false, onPost: { isInc, nDate, isExc in addTransaction(isInc: isInc, date: nDate, isExcluded: isExc) }, transactions: transactions, accounts: accounts)
        }
        .onAppear { loadHolidays() }
    }

    // MARK: - Subviews (コンパイラの負担を減らすための部品)
    ...
    private var dateHeaderView: some View {
        HStack(spacing: 4) {
            ...
            let isSunday = calendar.component(.weekday, from: selectedDate) == 1
            ...
            if let name = holidayName {
                Text(name).foregroundColor(Color(hex: themeBodyText)).padding(.leading, 4)
            }
            Spacer()
        }
        .font(.footnote)
    ...
    @ViewBuilder func monthGrid(for month: Date, width: CGFloat) -> some View {
        ...
        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 7), spacing: 0) {
            ForEach(0..<allDays.count, id: \.self) { index in
                dayCell(date: allDays[index], month: month)
            }
    ...
    // 【変更】isExcluded を反映
    func addTransaction(isInc: Bool, date: Date, isExcluded: Bool) { transactions.append(Transaction(amount: parseAmount(from: inputText), date: date, note: inputText, source: parseSourceName(from: inputText), isIncome: isInc, isExcludedFromBalance: isExcluded)) }
