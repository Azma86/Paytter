...
struct TransactionDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingEditSheet) { 
            PostView(inputText: $editLineText, isPresented: $isShowingEditSheet, initialDate: item.date, isExcludedInitial: item.isExcludedFromBalance ?? false, onPost: handleEditTransaction, transactions: transactions, accounts: accounts) 
        }
    }
    ...
    func handleEditTransaction(isInc: Bool, nDate: Date, isExc: Bool) {
        if let idx = transactions.firstIndex(where: { $0.id == item.id }) {
            ...
            for acc in accounts { if editLineText.contains("@\(acc.name)") { nSrc = acc.name } }
            var copy = transactions
            copy[idx] = Transaction(id: item.id, amount: nAmt, date: nDate, note: editLineText, source: nSrc, isIncome: isInc, isExcludedFromBalance: isExc)
            transactions = copy
        }
    }
}
