...
struct PostAttachedImage: Identifiable, Equatable {
    ...
    let data: Data
    let image: UIImage
    static func == (lhs: PostAttachedImage, rhs: PostAttachedImage) -> Bool { return lhs.id == rhs.id }
}

struct AttachedImageCell: View, Equatable {
    let id: UUID; let image: UIImage; let isDragged: Bool; let dragOffset: CGFloat; let onRemove: () -> Void
    static func == (lhs: AttachedImageCell, rhs: AttachedImageCell) -> Bool { lhs.id == rhs.id && lhs.isDragged == rhs.isDragged && lhs.dragOffset == rhs.dragOffset }
    var body: some View {
        ZStack(alignment: .topTrailing) {
            Image(uiImage: image).resizable().scaledToFill().frame(width: 80, height: 80).clipShape(RoundedRectangle(cornerRadius: 12))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(localImages) { item in
                        ...
                        AttachedImageCell(id: item.id, image: item.image, isDragged: isDragged, dragOffset: isDragged ? dragImageOffset : 0, onRemove: { localImages.removeAll(where: { $0.id == item.id }); attachedImages = localImages })
                        .equatable().gesture(DragGesture(coordinateSpace: .global).onChanged { val in handleImageDragChange(val, item: item) }.onEnded { _ in handleImageDragEnded() })
                    }
                }.padding(.horizontal)
            }.padding(.bottom, 8).onAppear { localImages = attachedImages }.onChange(of: attachedImages) { newImages in if draggedImageId == nil { localImages = newImages } }
        }
    }
    private func handleImageDragChange(_ value: DragGesture.Value, item: PostAttachedImage) {
        if draggedImageId != item.id { draggedImageId = item.id; dragImageTotalJump = 0 }
        ...
        if let idx = localImages.firstIndex(where: { $0.id == item.id }) {
            ...
            if dragImageOffset > threshold && idx < localImages.count - 1 { withAnimation(.interactiveSpring(response: 0.25, dampingFraction: 0.8, blendDuration: 0)) { localImages.swapAt(idx, idx + 1); dragImageTotalJump += jumpDistance; dragImageOffset -= jumpDistance } } else if dragImageOffset < -threshold && idx > 0 { withAnimation(.interactiveSpring(response: 0.25, dampingFraction: 0.8, blendDuration: 0)) { localImages.swapAt(idx, idx - 1); dragImageTotalJump -= jumpDistance; dragImageOffset += jumpDistance } }
        }
    }
    private func handleImageDragEnded() { withAnimation(.interactiveSpring()) { draggedImageId = nil; dragImageOffset = 0; dragImageTotalJump = 0 }; attachedImages = localImages }
}
...
struct PostView: View {
    ...
    var body: some View {
        NavigationView {
                ...
                VStack(spacing: 0) {
                    HStack(alignment: .top) {
                        ...
                        ZStack(alignment: .topLeading) {
                            CustomTextEditor(text: $inputText) { sym in insertAtCursor(sym); DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { updateSuggestionsForCursor() } }.foregroundColor(Color(hex: themeBarText)).onChange(of: inputText) { _ in updateSuggestionsForCursor() }
                            ...
                            HStack {
                                ForEach(attachedVideos, id: \.id) { video in
                                    ZStack(alignment: .topTrailing) {
                                        ...
                                        Button(action: { attachedVideos.removeAll(where: { $0.id == video.id }) }) { Image(systemName: "xmark.circle.fill").foregroundColor(.white).background(Circle().fill(Color.black.opacity(0.6))) }.padding(4)
                                    }
                                }
                            }.padding(.horizontal)
                        }.padding(.bottom, 8)
                    ...
                    HStack {
                        ...
                        // 【重要修正】onChange でコンパイラが迷わないよう、perform を明示
                        .onChange(of: selectedItems, perform: { newItems in
                            Task {
                                for item in newItems {
                                    // 【重要修正】UTType.movie と明示的に書くことでエラーを回避
                                    if item.supportedContentTypes.contains(where: { $0.conforms(to: UTType.movie) }) {
                                        ...
                                        if let data = try? await item.loadTransferable(type: Data.self), let uiImage = UIImage(data: data), let compressed = compressImage(uiImage), let compressedImage = UIImage(data: compressed) {
                                            DispatchQueue.main.async { if attachedImages.count < 4 { attachedImages.append(PostAttachedImage(data: compressed, image: compressedImage)) } }
                                        }
                                    }
                                }
            ...
            self.attachedImages = (initialImages ?? []).compactMap { data in if let img = UIImage(data: data) { return PostAttachedImage(data: data, image: img) }; return nil }
