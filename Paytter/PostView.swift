...
struct MovieTransferable: Transferable {
    ...
    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(importedContentType: .movie) { received in
            ...
            try? FileManager.default.removeItem(at: copy); try FileManager.default.copyItem(at: received.file, to: copy)
...
struct PostView: View {
    ...
    @AppStorage("theme_main") var themeMain: String = "#FF007AFF"; @AppStorage("theme_income") var themeIncome: String = "#FF19B219"; @AppStorage("theme_expense") var themeExpense: String = "#FFFF3B30"; @AppStorage("theme_bg") var themeBG: String = "#FFFFFFFF"; @AppStorage("theme_barText") var themeBarText: String = "#FF000000"; @AppStorage("theme_subText") var themeSubText: String = "#FF8E8E93"; @AppStorage("theme_bodyText") var themeBodyText: String = "#FF000000"
    @AppStorage("isDarkMode") var isDarkMode: Bool = false; @AppStorage("user_profiles_v1") var profiles: [UserProfile] = []
    
    @ObservedObject var lockManager = LockManager.shared
    @State private var postDate = Date(); @State private var isShowingDatePicker = false; @State private var isPickingTime = false; @State private var suggestions: [String] = []; @State private var isExcluded = false; @State private var selectedProfileId: UUID?
    ...
    var body: some View {
        NavigationView {
            ZStack(alignment: .bottom) {
                ...
                VStack(spacing: 0) {
                    HStack(alignment: .top) {
                        Menu { ForEach(profiles.filter { !($0.isPrivate ?? false) || lockManager.isUnlocked }.filter { !($0.isDeleted ?? false) }) { profile in Button(action: { selectedProfileId = profile.id }) { Text(profile.name) } } } label: { let currentProfile = profiles.first(where: { $0.id == selectedProfileId }) ?? profiles.first; if let iconData = currentProfile?.iconData, let uiImage = UIImage(data: iconData) { Image(uiImage: uiImage).resizable().scaledToFill().frame(width: 40, height: 40).clipShape(Circle()) } else { Image(systemName: "person.circle.fill").resizable().frame(width: 40, height: 40).foregroundColor(Color(hex: themeSubText)) } }
                        ZStack(alignment: .topLeading) { CustomTextEditor(text: $inputText) { sym in insertAtCursor(sym); DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { updateSuggestionsForCursor() } }.foregroundColor(Color(hex: themeBarText)).onChange(of: inputText) { _ in updateSuggestionsForCursor() }; if inputText.isEmpty { Text("どんな買い物をしましたか？").foregroundColor(.gray.opacity(0.7)).padding(.top, 8).padding(.leading, 5).allowsHitTesting(false) } }
                    }.padding().frame(maxHeight: .infinity)
                    
                    AttachedMediasDragView(attachedMedias: $attachedMedias)
                    
                    if !attachedFiles.isEmpty {
                        VStack(alignment: .leading, spacing: 4) { ForEach(attachedFiles, id: \.id) { file in HStack { Image(systemName: "doc.fill").foregroundColor(.gray); Text(file.originalFileName).lineLimit(1).truncationMode(.middle).foregroundColor(Color(hex: themeBodyText)); Spacer(); Text(file.formattedSize).foregroundColor(.gray); Button(action: { attachedFiles.removeAll(where: { $0.id == file.id }) }) { Image(systemName: "xmark").foregroundColor(.gray) } }.font(.caption).padding(8).background(Color.gray.opacity(0.1)).cornerRadius(8).padding(.horizontal) } }.padding(.bottom, 8)
                    }
                    
                    HStack {
                        PhotosPicker(selection: $selectedItems, maxSelectionCount: 4, matching: .any(of: [.images, .videos])) { Image(systemName: "photo.on.rectangle").font(.system(size: 20)).foregroundColor(Color(hex: themeMain)).padding(.trailing, 8) }
                        ...
                        Button(action: { isShowingFileImporter = true }) { Image(systemName: "doc.badge.plus").font(.system(size: 20)).foregroundColor(Color(hex: themeMain)).padding(.trailing, 8) }
                        Button(action: { isPickingTime = false; isShowingDatePicker = true }) { HStack(spacing: 4) { Image(systemName: "calendar.badge.clock"); Text(formatDate(postDate)) }.font(.footnote).padding(.horizontal, 12).padding(.vertical, 6).background(Color(hex: themeMain).opacity(0.1)).foregroundColor(Color(hex: themeMain)).cornerRadius(12) }
                        Spacer()
                ...
                if !suggestions.isEmpty { ScrollView(.horizontal, showsIndicators: false) { HStack(spacing: 10) { ForEach(suggestions, id: \.self) { suggestion in Button(action: { applySuggestion(suggestion) }) { Text(suggestion).font(.subheadline).fontWeight(.bold).foregroundColor(Color(hex: themeMain)).padding(.horizontal, 16).padding(.vertical, 10).background(Color(hex: themeBG)).overlay(RoundedRectangle(cornerRadius: 20).stroke(Color(hex: themeMain).opacity(0.5), lineWidth: 1)).cornerRadius(20) } } }.padding(.horizontal).padding(.vertical, 8) }.background(Color(hex: themeBG).opacity(0.95)).shadow(color: Color.black.opacity(0.1), radius: 5, x: 0, y: -2).offset(y: -44).zIndex(10) }
            }
            .navigationBarItems(
            ...
            .fileImporter(isPresented: $isShowingFileImporter, allowedContentTypes: [UTType.data], allowsMultipleSelection: true) { result in if case .success(let urls) = result { for url in urls { let isSecured = url.startAccessingSecurityScopedResource(); defer { if isSecured { url.stopAccessingSecurityScopedResource() } }; if let savedName = MediaManager.shared.saveMedia(from: url) { let attrs = try? FileManager.default.attributesOfItem(atPath: url.path); let size = attrs?[.size] as? Int64 ?? 0; attachedFiles.append(AttachedFile(originalFileName: url.lastPathComponent, localFileName: savedName, fileSize: size)) } } } }
        }.onAppear { 
