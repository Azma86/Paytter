...
struct PostView: View {
        ...
        NavigationView {
                                ...
                                ForEach(suggestions, id: \.self) { suggestion in
                                    Button(action: { applySuggestion(suggestion) }) {
                                        VStack(alignment: .leading) { 
                                            Text(suggestion).font(.body).foregroundColor(Color(hex: themeBarText)).padding(.vertical, 12).padding(.horizontal, 20)
                                            Divider() 
                                        }
                                    }
                                }
            ...
            .navigationBarItems(
                ...
                trailing: HStack(spacing: 12) {
                    Button(action: { onPost(false, postDate, isExcluded); isPresented = false }) {
                        Text("支出").font(.subheadline).fontWeight(.bold).frame(width: 60, height: 34).background(Color(hex: themeExpense).opacity(0.8)).foregroundColor(.white).cornerRadius(17)
                    }
                    Button(action: { onPost(true, postDate, isExcluded); isPresented = false }) {
                        Text("収入").font(.subheadline).fontWeight(.bold).frame(width: 60, height: 34).background(Color(hex: themeIncome)).foregroundColor(.white).cornerRadius(17)
                    }
                }
            )
            .sheet(isPresented: $isShowingDatePicker) {
                NavigationView {
                    ZStack {
                        Color(hex: themeBG).ignoresSafeArea()
                        VStack { DatePicker("日時を選択", selection: $postDate, displayedComponents: isPickingTime ? .hourAndMinute : .date).datePickerStyle(.wheel).labelsHidden().environment(\.locale, Locale(identifier: "ja_JP")).background(Color.clear) }
                    }
                    .navigationTitle(isPickingTime ? "時刻の指定" : "日付の指定").navigationBarTitleDisplayMode(.inline)
                    .navigationBarItems(
                        leading: Button(isPickingTime ? "日付に切り替え" : "時刻に切り替え") { withAnimation { isPickingTime.toggle() } }.foregroundColor(Color(hex: themeMain)), 
                        trailing: Button("完了") { isShowingDatePicker = false }.foregroundColor(Color(hex: themeMain))
                    )
                }.preferredColorScheme(isDarkMode ? .dark : .light).presentationDetents([.height(350)])
            }
        }.onAppear { self.postDate = initialDate; self.isExcluded = isExcludedInitial }
    }
    ...
    func updateSuggestionsForCursor() { 
        ...
        let cursorLoc = tv.selectedRange.location; let text = tv.text ?? ""; let prefixText = String(text.prefix(cursorLoc))
    ...
    func applySuggestion(_ suggestion: String) { 
        ...
        if let lastWord = words.last { 
            ...
            let startIdx = text.index(text.startIndex, offsetBy: rangeStart); let endIdx = text.index(text.startIndex, offsetBy: cursorLoc)
