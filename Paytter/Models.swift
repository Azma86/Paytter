...
import Combine
import AVFoundation // 【新規】動画サムネイル生成に必要
...
// 【新規】動画とファイル用のモデル
struct AttachedVideo: Codable, Equatable {
    var id = UUID()
...
// 【新規】iPhone内のDocumentフォルダに重いファイルを逃がす仕組み
class MediaManager {
    ...
    func saveMedia(from url: URL) -> String? {
        ...
        do { try FileManager.default.copyItem(at: url, to: destURL); return fileName }
        catch { print("Save error:", error); return nil }
    }
}
...
struct Transaction: Identifiable, Codable, Equatable {
    ...
    // 【新規】Transactionに動画とファイルの情報を持たせる
