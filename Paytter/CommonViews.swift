...
class PlayerView: UIView { var player: AVPlayer? { get { playerLayer.player } set { playerLayer.player = newValue } }; var playerLayer: AVPlayerLayer { return layer as! AVPlayerLayer }; override static var layerClass: AnyClass { return AVPlayerLayer.self } }
...
struct MediaFullScreenView: View {
    ...
    @State private var loadedImage: UIImage? = nil // 画像読み込みによる暗転フリッカーを防ぐキャッシュ
    ...
    @State private var isSeeking: Bool = false // シーク中の綱引きを防ぐフラグ
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            // ヘッダーUI（画像の時は黒い背景をなくしてスッキリと）
            if showUI {
                HStack(spacing: 16) {
                    Button(action: { presentationMode.wrappedValue.dismiss() }) {
                        Image(systemName: "chevron.left").font(.title3).foregroundColor(.white).padding(10).background(Color.black.opacity(0.4).clipShape(Circle()))
                    }
                    
                    let displayName = media.originalFileName ?? (media.localFileName.isEmpty ? "添付画像" : media.localFileName)
                    Text(displayName).font(.subheadline).foregroundColor(.white).lineLimit(1).truncationMode(.middle).frame(maxWidth: .infinity, alignment: .leading)
                    
                    Button(action: shareMedia) { Image(systemName: "square.and.arrow.up").font(.title3).foregroundColor(.white).padding(10).background(Color.black.opacity(0.4).clipShape(Circle())) }
                    Button(action: saveMedia) { Image(systemName: "arrow.down.to.line").font(.title3).foregroundColor(.white).padding(10).background(Color.black.opacity(0.4).clipShape(Circle())) }
                }
                .padding(.horizontal, 16).padding(.top, safeAreaTop).padding(.bottom, 16)
                .background(media.type == .video ? LinearGradient(gradient: Gradient(colors: [Color.black.opacity(0.7), Color.clear]), startPoint: .top, endPoint: .bottom) : nil)
                ...
                VStack {
                    ...
                    HStack(spacing: 16) {
                        Button(action: {
                            ...
                            Image(systemName: isPlaying ? "pause.fill" : "play.fill").font(.title2).foregroundColor(.white).frame(width: 24, height: 24)
                        }
                        ...
                        Text("-" + formatTime(duration - currentTime)).font(.caption2).foregroundColor(.white).monospacedDigit().frame(width: 40, alignment: .trailing)
                    }
                    .padding(.horizontal, 24).padding(.vertical, 24).padding(.bottom, safeAreaBottom > 0 ? safeAreaBottom : 16)
        ...
        .onAppear {
            if media.type == .video { setupPlayer() } else {
                // 画像表示時のフリッカー（暗転）を防ぐため、非同期で1度だけ読み込む
        ...
        .alert(isPresented: $showSaveAlert) { Alert(title: Text("保存"), message: Text(saveAlertMessage), dismissButton: .default(Text("OK"))) }
    }
    ...
    var safeAreaTop: CGFloat { UIApplication.shared.windows.first?.safeAreaInsets.top ?? 20 }
    ...
    func saveMedia() {
        if media.localFileName.isEmpty {
            if let data = media.thumbnailData, let image = UIImage(data: data) {
                ...
                saveAlertMessage = "画像をカメラロールに保存しました"; showSaveAlert = true
            }
            return
        }
        let url = MediaManager.shared.getMediaURL(fileName: media.localFileName)
        if media.type == .image {
            ...
            if UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(url.path) {
                ...
                saveAlertMessage = "動画をカメラロールに保存しました"; showSaveAlert = true
            }
        }
    }
}
...
struct TimelineImageGrid: View {
    let images: [Data]; var cornerRadius: CGFloat = 12; var maxHeight: CGFloat = 160
    var body: some View { let count = images.count; Group { if count == 1 { imgView(images[0]) } else if count == 2 { HStack(spacing: 4) { imgView(images[0]); imgView(images[1]) } } else if count == 3 { HStack(spacing: 4) { imgView(images[0]); VStack(spacing: 4) { imgView(images[1]); imgView(images[2]) } } } else if count >= 4 { VStack(spacing: 4) { HStack(spacing: 4) { imgView(images[0]); imgView(images[1]) }; HStack(spacing: 4) { imgView(images[2]); imgView(images[3]) } } } }.frame(height: maxHeight).clipShape(RoundedRectangle(cornerRadius: cornerRadius)).overlay(RoundedRectangle(cornerRadius: cornerRadius).stroke(Color.gray.opacity(0.2), lineWidth: 1)) }
    @ViewBuilder func imgView(_ data: Data) -> some View { if let uiImage = ImageCache.shared.image(for: data) { Image(uiImage: uiImage).resizable().scaledToFill().frame(minWidth: 0, maxWidth: .infinity, minHeight: 0, maxHeight: .infinity).clipped() } else { Color.gray.opacity(0.1) } }
}
...
struct BalanceView: View {
    ...
    @AppStorage("theme_income") var themeIncome: String = "#FF19B219"; @AppStorage("theme_expense") var themeExpense: String = "#FFFF3B30"; @AppStorage("theme_subText") var themeSubText: String = "#FF8E8E93"
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if hideContent { Text("鍵アカウントによる投稿です").font(.subheadline).foregroundColor(Color(hex: themeSubText)) } else {
                    ...
                    if let files = item.attachedFiles, !files.isEmpty { VStack(alignment: .leading, spacing: 4) { ForEach(files, id: \.id) { file in HStack { Image(systemName: "doc.fill").foregroundColor(.gray); Text(file.originalFileName).lineLimit(1).truncationMode(.middle).foregroundColor(Color(hex: themeBodyText)); Spacer(); Text("\(file.fileExtension) · \(file.formattedSize)").foregroundColor(.gray) }.font(.caption).padding(8).background(Color.gray.opacity(0.1)).cornerRadius(8) } }.padding(.top, 4) }
                }
            }
...
struct HighlightedText: View {
    ...
    @AppStorage("theme_income") var themeIncome: String = "#FF19B219"; @AppStorage("theme_expense") var themeExpense: String = "#FFFF3B30"
