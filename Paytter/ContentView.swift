...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            recalculateBalances(); updateAppearance(); syncHomeItems()
            if !lockManager.isUnlocked && !lockManager.passcode.isEmpty { lockManager.promptUnlock() }
        }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarLeading) {
                    if !lockManager.passcode.isEmpty {
                        Button(action: { if lockManager.isUnlocked { lockManager.lock() } else { lockManager.promptUnlock() } }) { Image(systemName: lockManager.isUnlocked ? "lock.open.fill" : "lock.fill").foregroundColor(Color(hex: themeMain)) }
                    }
                }
    ...
    func handlePostTransaction(isInc: Bool, date: Date, isExc: Bool, profileId: UUID?) { transactions.append(Transaction(amount: parseAmount(from: inputText), date: date, note: inputText, source: parseSourceName(from: inputText), isIncome: isInc, isExcludedFromBalance: isExc, profileId: profileId)) }
    
    @ViewBuilder private func homeHeaderItem(for item: HomeItem) -> some View { Group { switch item { case .totalAssets: let b = accounts.reduce(0) { $0 + $1.balance }; let d = accounts.reduce(0) { $0 + $1.diffAmount }; BalanceView(title: "総資産", amount: b, color: Color(hex: themeBodyText), diff: d); case .account(let a): if let c = accounts.first(where: { $0.id == a.id }) { BalanceView(title: c.name, amount: c.balance, color: Color(hex: themeBodyText), diff: c.diffAmount) }; case .group(let g): if let c = groups.first(where: { $0.id == g.id }) { let accs = accounts.filter { c.accountIds.contains($0.id) }; let b = accs.reduce(0) { $0 + $1.balance }; let d = accs.reduce(0) { $0 + $1.diffAmount }; BalanceView(title: c.name, amount: b, color: Color(hex: themeBodyText), diff: d) } } } }
    ...
    func recalculateBalances() { var tempAccounts = accounts; for i in 0..<tempAccounts.count { var cur = 0; for tx in transactions where tx.source == tempAccounts[i].name { if tx.isExcludedFromBalance == true { continue }; cur += (tx.isIncome ? tx.amount : -tx.amount) }; tempAccounts[i].diffAmount = cur - tempAccounts[i].balance; tempAccounts[i].balance = cur }; accounts = tempAccounts; BackupManager.saveFullBackup(data: createFullBackupData(), isManual: false) }
