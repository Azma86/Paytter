...
struct ContentView: View {
    ...
    private var settingTab: some View { 
        NavigationView { 
            ...
            .alert("バックアップの復元", isPresented: $isShowingRestoreConfirm) { 
                Button("キャンセル", role: .cancel) {}; Button("復元する", role: .destructive) { if let t = BackupManager.loadTransactions(isManual: isRestoringManual), let a = BackupManager.loadAccounts(isManual: isRestoringManual) { transactions = t; accounts = a; recalculateBalances(); completionMessage = "復元が完了しました。"; isShowingCompletionAlert = true } } 
            ...
            .alert("外部読込", isPresented: $isShowingImportConfirm) {
                ...
                Button("復元する", role: .destructive) { if let d = pendingImportData { transactions = d.0; accounts = d.1; recalculateBalances(); completionMessage = "外部バックアップの読込が完了しました。"; isShowingCompletionAlert = true }; pendingImportData = nil }
    ...
    func resetAll() { transactions = []; accounts = [Account(name: "お財布", balance: 0, type: .wallet), Account(name: "口座", balance: 0, type: .bank), Account(name: "ポイント", balance: 0, type: .point)]; monthlyBudget = 50000; completionMessage = "リセット完了"; isShowingCompletionAlert = true }
    ...
    private func updateViewHierarchy(_ vc: UIViewController?) {
        ...
        if let tab = vc as? UITabBarController {
            ...
            if #available(iOS 15.0, *) { tab.tabBar.scrollEdgeAppearance = UITabBar.appearance().scrollEdgeAppearance }
        }
...
struct BalanceView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if diff != 0 {
                Text(diff > 0 ? "+\(diff)" : "\(diff)").font(.system(size: 8, weight: .bold, design: .rounded)).foregroundColor(diff > 0 ? .green : .red)
            }
