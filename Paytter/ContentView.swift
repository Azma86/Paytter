...
// 【新規】フルスクリーン表示と拡大縮小、カスタムヘッダーを備えたView
            ...
            GeometryReader { proxy in
                if media.type == .video {
                    ...
                    VideoPlayer(player: AVPlayer(url: url))
                        ...
                        .onTapGesture { withAnimation { showHeader.toggle() } }
                } else {
                    if let originalImage = MediaManager.shared.loadImage(fileName: media.localFileName) {
                        Image(uiImage: originalImage)
                            ...
                            .onTapGesture(count: 1) { withAnimation { showHeader.toggle() } }
                            .frame(width: proxy.size.width, height: proxy.size.height)
                        ...
                        Image(uiImage: img)
                            ...
                            .gesture(MagnificationGesture().onChanged { val in let delta = val / lastScale; lastScale = val; scale = min(max(scale * delta, 1), 5) }.onEnded { _ in lastScale = 1.0; if scale <= 1.0 { withAnimation { offset = .zero; lastOffset = .zero } } })
                            .simultaneousGesture(DragGesture().onChanged { val in if scale > 1.0 { offset = CGSize(width: lastOffset.width + val.translation.width, height: lastOffset.height + val.translation.height) } }.onEnded { _ in lastOffset = offset })
                            .onTapGesture(count: 2) { withAnimation { if scale > 1.0 { scale = 1.0; offset = .zero; lastOffset = .zero } else { scale = 2.0 } } }
                            .onTapGesture(count: 1) { withAnimation { showHeader.toggle() } }
                            .frame(width: proxy.size.width, height: proxy.size.height)
                    } else {
                        Text("画像を読み込めません").foregroundColor(.white).frame(maxWidth: .infinity, maxHeight: .infinity)
                    }
                }
            }
            .clipped()
            ...
            if showHeader {
                HStack(spacing: 16) {
                    Button(action: { presentationMode.wrappedValue.dismiss() }) {
                        Image(systemName: "chevron.left").font(.title3).foregroundColor(.white).padding(8)
                    }
                    ...
                    Button(action: shareMedia) {
                        Image(systemName: "square.and.arrow.up").font(.title3).foregroundColor(.white).padding(8)
                    }
                    
                    Button(action: saveMedia) {
                        Image(systemName: "arrow.down.to.line").font(.title3).foregroundColor(.white).padding(8)
                    }
                }
...
// 旧：TimelineImageGrid は互換性のため残します
