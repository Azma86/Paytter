...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert(item: $activeAlert) { type in
            ...
            case .reset:
                return Alert(
                    title: Text("全リセット"),
                    message: Text("全てのデータとユーザー設定を初期化します。"),
                    primaryButton: .destructive(Text("リセット")) { resetAll() },
                    secondaryButton: .cancel(Text("キャンセル"))
                )
            case .restore:
                let dateStr = BackupManager.getBackupDate(isManual: isRestoringManual)
                return Alert(
                    title: Text("バックアップの復元"),
                    message: Text("保存日時: \(dateStr)\nデータを復元しますか？"),
                    primaryButton: .destructive(Text("復元")) {
                        if let b = BackupManager.loadFullBackup(isManual: isRestoringManual) {
                            applyFullBackup(b)
                            activeAlert = .completion("復元完了")
                        } else if let t = BackupManager.loadTransactions(isManual: isRestoringManual),
                                  let a = BackupManager.loadAccounts(isManual: isRestoringManual) {
                            transactions = t
                            accounts = a
                            recalculateBalances()
                            activeAlert = .completion("復元完了(旧形式)")
                        }
                    },
                    secondaryButton: .cancel(Text("キャンセル"))
                )
            case .save:
                return Alert(
                    title: Text("バックアップの保存"),
                    message: Text("現在のすべてのデータで上書きしますか？"),
                    primaryButton: .default(Text("保存")) {
                        BackupManager.saveFullBackup(data: createFullBackupData(), isManual: true)
                        activeAlert = .completion("保存完了")
                    },
                    secondaryButton: .cancel(Text("キャンセル"))
                )
            case .importConfirm:
                return Alert(
                    title: Text("外部データの読込"),
                    message: Text("保存日時: \(pendingImportData?.backupDate ?? "")\nデータを上書きしますか？"),
                    primaryButton: .destructive(Text("読み込む")) {
                        if let d = pendingImportData {
                            applyFullBackup(d)
                            activeAlert = .completion("読込完了")
                        }
                ...
                if !isHomeEditMode {
                    Button(action: {
                        ...
                        Image(systemName: "plus")
                            ...
                            .background(Color(hex: themeMain))
                            .clipShape(Circle())
                    }
                    .padding(20)
                    .padding(.bottom, 10)
    ...
    private var walletTab: some View {
        NavigationView {
            ...
            .sheet(isPresented: $isShowingAccountCreator) {
                AccountCreateView(accounts: $accounts, transactions: $transactions)
            }
            .sheet(isPresented: $isShowingGroupCreator) {
                AccountGroupCreateView(groups: $groups, accounts: $accounts)
            }
            ...
            .alert("グループの削除", isPresented: $isShowingGroupDeleteAlert) {
                ...
                Button("削除", role: .destructive) {
                    if let grp = groupToDelete {
                        groups.removeAll(where: { $0.id == grp.id })
                    }
                    groupToDelete = nil
                }
            }
            ...
            ZStack {
                ...
                List {
                    ...
                    Section(header: Text("データ管理").foregroundColor(Color(hex: themeSubText))) {
                        Button("全データをリセット", role: .destructive) {
                            activeAlert = .reset
                        }
                    }
                    .listRowBackground(Color(hex: themeBG).opacity(0.5))
    ...
    func createFullBackupData() -> FullBackupData {
        return FullBackupData(
            transactions: transactions, accounts: accounts, groups: groups, profiles: profiles,
            ...
            themeSubText: themeSubText, showTotalAssets: showTotalAssets, homeDisplayOrder: homeDisplayOrder,
    ...
    func applyFullBackup(_ backup: FullBackupData) {
        ...
        themeMain = backup.themeMain
        themeIncome = backup.themeIncome
        themeExpense = backup.themeExpense
        themeHoliday = backup.themeHoliday
        themeSaturday = backup.themeSaturday
        themeBG = backup.themeBG
        themeBarBG = backup.themeBarBG
        themeBarText = backup.themeBarText
        themeTabAccent = backup.themeTabAccent
        themeBodyText = backup.themeBodyText
        themeSubText = backup.themeSubText
        showTotalAssets = backup.showTotalAssets
    ...
    func handleImport(from url: URL) {
        ...
        if let fd = try? JSONDecoder().decode(FullBackupData.self, from: data) {
            self.pendingImportData = fd
            self.activeAlert = .importConfirm
            ...
            self.pendingImportData = FullBackupData(
                ...
                monthlyBudget: fd.monthlyBudget, isDarkMode: fd.isDarkMode, themeMain: fd.themeMain,
                themeIncome: fd.themeIncome, themeExpense: fd.themeExpense, themeHoliday: fd.themeHoliday,
                themeSaturday: fd.themeSaturday, themeBG: fd.themeBG, themeBarBG: fd.themeBarBG,
                themeBarText: fd.themeBarText, themeTabAccent: fd.themeTabAccent, themeBodyText: fd.themeBodyText,
                themeSubText: fd.themeSubText, showTotalAssets: fd.showTotalAssets, homeDisplayOrder: fd.homeDisplayOrder,
                backupDate: "以前の形式"
            )
            self.activeAlert = .importConfirm
        }
    ...
    func resetAll() {
        ...
        groups = []
        monthlyBudget = 50000
        profiles = [UserProfile(name: "むつき", userId: "Mutsuki_dev")]
        recalculateBalances()
    ...
    func parseSourceName(from t: String) -> String {
        for acc in accounts {
            if t.contains("@\(acc.name)") { return acc.name }
        }
    ...
    private func updateViewHierarchy(_ vc: UIViewController?) {
        ...
        if let nav = vc as? UINavigationController {
            ...
            nav.navigationBar.scrollEdgeAppearance = UINavigationBar.appearance().standardAppearance
